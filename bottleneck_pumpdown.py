...
def be_step(P1, P2, C, Q1, Q2, Pu, S, V1, V2, dt):
    """One backward-Euler step for one gas species (Q1 -> manifold, Q2 -> chamber).

    S = pump speed for this species over the step; Pu = that pump's ultimate as
    a back-leak. Pu is 0 for the roughers because their ultimate already lives
    inside S (back-leakage past the seals is a real loss of swept volume); for
    the turbo it is a genuine back-leak through the blade stack.
    """
    a11 = 1 + dt * (C + S) / V1
    a12 = -dt * C / V1
    a21 = -dt * C / V2
...
def simulate(d, L, V1, V2, Px=0.1, rp=DEF_RP, tp=DEF_TP, gb=False,
             og=False, ogA=OG_A0, ogV=OG_V0, lk=False, Ql=1e-6, ww=True):
    ...
    while P2a + P2w > Tb["Pu"] * 1.02 and t < T_END and steps < 60000:
        steps += 1
        ...
        n1a, n2a = be_step(P1a, P2a, cv + cm, 0.0, (Ql if lk else 0.0), Pua, Sa, V1, V2, dt)
        n1w, n2w = be_step(P1w, P2w, cv + cm * FW, qw1, qw2, 0.0, Sw, V1, V2, dt)
    ...
    # land on the 20-day window edge: hold the last state out to T_END if the
    # run bottomed out early, else trim the final step's overshoot
    if t < T_END:
        t = T_END
        rec(pump_speeds(crossed, P1a, P1w, rp, tp, gb)[0], None)
    else:
...
def simulate_ideal(V1, V2, Px=0.1, rp=DEF_RP, tp=DEF_TP, gb=False,
                   og=False, ogA=OG_A0, ogV=OG_V0, lk=False, Ql=1e-6, ww=True):
    ...
    rec()
    last_rec, steps = 0.0, 0
    while Pa + Pw > Tb["Pu"] * 1.02 and t < T_END and steps < 60000:
        steps += 1
        if not crossed and cross_now(Pa + Pw, Pa + Pw, Px, rp, gb):
            crossed = True
        Sa, Sw = pump_speeds(crossed, Pa, Pw, rp, tp, gb)
        Pua = Tb["Pu"] if crossed else 0.0
        Qw = (q_og(t, ogA, ogV) if og else 0.0) + ((q_dw(t) + q_man(t)) if ww else 0.0)
        na = (Pa + dt * (Sa * Pua + (Ql if lk else 0.0)) / V) / (1 + dt * Sa / V)
        nw = (Pw + dt * Qw / V) / (1 + dt * Sw / V)
        ...
        dt = min(dt * 1.2, 0.02 * t + 1e-7)
    rec()
    # land on the 20-day window edge, matching simulate()
    if t < T_END:
        t = T_END
        rec()
    else:
...
def check():
    ...
    print("\n-- wall/manifold water always on: hot-wall pool "
          f"{DW_I0:.2e} Torr*L (tau 30 min); manifold q(1 h) = {q_man(3600.0):.2e} Torr*L/s --")
    print("-- with 1e-6 Torr*L/s air leak --")
    sim = simulate(d, L, V1, V2, Px, DEF_RP, tp, lk=True, Ql=1e-6)
    cm = cond_mol(d, L)
    print(f"  floor P2 = {sim['P2'][-1]:.3g} Torr vs predicted Q/S_eff = "
          f"{1e-6*(1/Tb['Sn2'] + 1/cm):.3g} Torr "
...
def interactive():
    ...
    def update(_=None):
        Px, d, L, V1, V2, ogA, ogV = (10 ** s.val for s in sliders)
        gb, og = ck.get_status()
        ...
        sim = simulate(d, L, V1, V2, Px, rp, tp, gb, og, ogA, ogV)
        T, P1, P2, W1, W2 = sim["T"], sim["P1"], sim["P2"], sim["W1"], sim["W2"]
        ix, iy = simulate_ideal(V1, V2, Px, rp, tp, gb, og, ogA, ogV)
